...
const inputCls = 'w-full bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-lg px-3 py-2 text-sm text-zinc-900 dark:text-zinc-100 placeholder-zinc-400 dark:placeholder-zinc-500 focus:outline-none focus:ring-2 focus:ring-orange-500/30 focus:border-orange-500'
const selectCls = 'w-full bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-lg px-3 py-1.5 text-sm text-zinc-700 dark:text-zinc-200 focus:outline-none focus:ring-2 focus:ring-orange-500/30 cursor-pointer'

// blank state - reset to this after submit
const EMPTY = {
  title: '',
  description: '',
  priority: 'medium',
...
export default function TaskForm({ onAddTask, categories }) {
  ...
  function handleSubmit(e) {
    ...
    // figure out the final category
    let cat = form.category
    if (form.category === '_custom') {
      cat = form.customCategory.trim() || 'General'
    }
      ...
      <div className="flex gap-2">
        <div className="flex-1">
          ...
          <input
            ...
            type="text"
            value={form.title}
            onChange={e => { setField('title', e.target.value); setErr('') }}
            onFocus={() => setOpen(true)}
            placeholder="What needs to be done?"
            className={`${inputCls} ${err ? 'border-red-400 dark:border-red-600' : ''}`}
          />
      ...
      {open && (
        <div className="mt-4 flex flex-col gap-3 border-t border-zinc-100 dark:border-zinc-800 pt-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            ...
            <div>
              <label htmlFor="task-cat" className="block text-xs font-medium text-zinc-500 dark:text-zinc-400 mb-1">
                Category
              </label>
              <select
                id="task-cat"
          ...
          {form.category === '_custom' && (
            <div>
              <label htmlFor="custom-cat-name" className="block text-xs font-medium text-zinc-500 dark:text-zinc-400 mb-1">
                New category name
              </label>
              <input
                id="custom-cat-name"
